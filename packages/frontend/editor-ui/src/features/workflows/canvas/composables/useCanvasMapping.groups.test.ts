...
describe('computeMemberRectFromStore', () => {
	it('returns the bounding rect of all members', () => {
		const getById = nodeStore(makeNode('a', 0, 0), makeNode('b', 300, 100));
		const rect = computeMemberRectFromStore(['a', 'b'], getById);
		expect(rect.x).toBe(0);
		expect(rect.y).toBe(0);
		expect(rect.width).toBe(400); // 300 + 100 default width
		expect(rect.height).toBe(200); // 100 + 100 default height
	});
...
describe('mapGroupsToVueFlowNodes', () => {
	...
	it('expanded width matches member rect + 2 * GROUP_PADDING_X', () => {
		...
		// member rect width = (400 + 100) - 100 = 400
		expect(out[0].width).toBe(400 + 2 * GROUP_PADDING_X);
	});

	it('marks the node selectable: false and connectable: false', () => {
		const out = setup(true);
