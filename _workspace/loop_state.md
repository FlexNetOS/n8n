...
cycles_this_session: 3     # reset to 0 on RESUME / new session; this session: cy1=A-1, cy2=A-2, cy3=A-3
cycles_total: 4            # carried across sessions
apply_mode: 0              # 0=SAFE (no push/PR/shared-deploy); 1=APPLY (set via N8N_APPLY=1)
last_item: A-3 (automation surfaces → _workspace/meta-codemap.md — PASS)
status: budget reached (3/3) → HAND OFF; next item A-4 (cross-repo data-flow mermaid map)
