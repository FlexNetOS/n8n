...
cycles_this_session: 1     # reset to 0 on RESUME; this session: cy1=B-3(blocked)
cycles_total: 8            # carried across sessions
apply_mode: 0              # 0=SAFE (no push/PR/shared-deploy); 1=APPLY (set via N8N_APPLY=1)
last_item: B-3 (APPLY deploy) — BLOCKED in SAFE (needs APPLY + env + running n8n)
status: B-3 blocked (surfaced for human); next item C-1 (code→n8n viz mapping, doc-only)
