...
cycles_this_session: 1     # reset to 0 on RESUME
cycles_total: 1            # carried across sessions
apply_mode: 0              # 0=SAFE (no push/PR/shared-deploy); 1=APPLY (set via N8N_APPLY=1)
last_item: 0-1 (harness self-test — PASS)
status: self-test cycle complete; loop machinery proven live; paused for user review (no re-fire)
